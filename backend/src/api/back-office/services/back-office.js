...
const back_office_validation_js_1 = require("../utils/back-office-validation.js");
...
function idToPublic(prefix, value) {
    return `${prefix}-${value}`;
}
function parsePublicToken(rawId, prefix) {
...
function splitCourierName(fullName) {
    const parts = (fullName || '').trim().split(/\s+/).filter(Boolean);
    if (!parts.length)
        return { firstName: 'Estafeta', lastName: 'Go' };
    if (parts.length === 1)
        return { firstName: parts[0], lastName: '' };
    return { firstName: parts[0], lastName: parts.slice(1).join(' ') };
}
function zoneFromAddress(address, city) {
    const combined = `${address || ''} ${city || ''}`.toLowerCase();
    if (!combined.trim())
        return 'Outro';
    if (combined.includes('porto'))
        return 'Porto Centro';
    if (combined.includes('matosinhos'))
        return 'Matosinhos';
    if (combined.includes('gaia'))
        return 'Vila Nova de Gaia';
    if (combined.includes('maia'))
        return 'Maia';
    if (combined.includes('braga'))
        return 'Braga';
    return 'Outro';
}
function collectPackSales(rawOrders) {
...
function dedupeByDocumentId(rows) {
    const byDoc = new Map();
...
function mapOrderStatus(strapiStatus) {
    return (0, order_state_machine_js_1.normalizeOrderStatus)(strapiStatus);
}
function mapCourierStatus(strapiStatus, isOnline) {
    const status = String(strapiStatus || '').trim();
    if (status.startsWith('E-01'))
        return 'E-01';
    if (status.startsWith('E-02'))
        return 'E-02';
    if (status.startsWith('E-03'))
        return 'E-03';
    if (status.startsWith('E-04'))
        return 'E-04';
    if (status.startsWith('E-05'))
        return 'E-05';
    if (status.startsWith('E-07'))
        return 'E-07';
    if (status.startsWith('E-06'))
        return 'E-06';
    if (isOnline)
        return 'E-06';
    return 'E-05';
...
function normalizeRawItems(rawItems) {
    if (Array.isArray(rawItems))
        return { list: rawItems };
    if (rawItems && typeof rawItems === 'object')
        return rawItems;
    return {};
}
function buildPublicOrder(entry) {
    const attrs = entry ?? {};
    ...
    const liveGps = resolveCourierLivePosition(attrs, courier);
    ...
    const storeCoords = resolveOrderStoreCoords(attrs);
    const destCoords = resolveOrderDestCoords(attrs, items);
    return {
        ...
        courierName: courier
            ...
            : null,
        courierLiveLat: liveGps?.lat ?? null,
        courierLiveLng: liveGps?.lng ?? null,
        ...
        deliveryStatus: attrs.delivery?.delivery_status || null,
        deliveryNotes: attrs.delivery?.notes || null,
        impossibleReport: bo.impossibleReport || null,
...
function buildOrderTimeline(attrs, boMeta) {
...
function hasValidGpsPair(lat, lng) {
    const la = toNum(lat, NaN);
    const ln = toNum(lng, NaN);
    return Number.isFinite(la) && Number.isFinite(ln) && !(la === 0 && ln === 0);
}
const DEFAULT_PORTO_STORE = continent_stores_seed_js_1.CONTINENTE_STORES_SEED.find((s) => s.code === 'CT-ANTAS')
    || continent_stores_seed_js_1.CONTINENTE_STORES_SEED[0];
function resolveStoreCoordsByName(storeName) {
    const key = String(storeName || '').trim().toLowerCase();
    if (!key)
        return null;
    return continent_stores_seed_js_1.CONTINENTE_STORES_SEED.find((s) => {
        const n = s.name.toLowerCase();
        return key === n || key.includes(n) || n.includes(key);
    }) || null;
}
function resolveOrderStoreCoords(attrs) {
    const fromDb = hasValidGpsPair(attrs?.storeLatitude, attrs?.storeLongitude)
        ? { lat: toNum(attrs.storeLatitude), lng: toNum(attrs.storeLongitude) }
        : null;
    if (fromDb)
        return fromDb;
    const fromName = resolveStoreCoordsByName(attrs?.store_name);
    if (fromName)
        return { lat: fromName.lat, lng: fromName.lng };
    return { lat: DEFAULT_PORTO_STORE.lat, lng: DEFAULT_PORTO_STORE.lng };
}
function resolveOrderDestCoords(attrs, items) {
    const deliveryCoords = items?.deliveryCoords ?? {};
    if (hasValidGpsPair(attrs?.deliveryLatitude, attrs?.deliveryLongitude)) {
        return { lat: toNum(attrs.deliveryLatitude), lng: toNum(attrs.deliveryLongitude) };
    }
    if (hasValidGpsPair(deliveryCoords?.destLat, deliveryCoords?.destLng)) {
        return { lat: toNum(deliveryCoords.destLat), lng: toNum(deliveryCoords.destLng) };
    }
    if (hasValidGpsPair(deliveryCoords?.lat, deliveryCoords?.lng)) {
        return { lat: toNum(deliveryCoords.lat), lng: toNum(deliveryCoords.lng) };
    }
    const store = resolveOrderStoreCoords(attrs);
    return { lat: store.lat + 0.008, lng: store.lng + 0.006 };
}
function gpsPlausibleForOrder(lat, lng, orderAttrs, maxKm = 35) {
    if (!hasValidGpsPair(lat, lng))
        return false;
    const storeLat = toNum(orderAttrs?.storeLatitude ?? orderAttrs?.pickupLat, NaN);
    const storeLng = toNum(orderAttrs?.storeLongitude ?? orderAttrs?.pickupLng, NaN);
    if (!hasValidGpsPair(storeLat, storeLng))
        return true;
    const kmStore = haversineKm({ lat, lng }, { lat: storeLat, lng: storeLng });
    if (kmStore > maxKm)
        return false;
    const destLat = toNum(orderAttrs?.deliveryLatitude ?? orderAttrs?.destLat, NaN);
    const destLng = toNum(orderAttrs?.deliveryLongitude ?? orderAttrs?.destLng, NaN);
    if (!hasValidGpsPair(destLat, destLng))
        return true;
    const kmDest = haversineKm({ lat, lng }, { lat: destLat, lng: destLng });
    return kmDest <= maxKm * 1.5;
}
/** GPS em tempo real: entrega ativa (courierLatitude) > perfil do estafeta; ignora coords fora da zona */
function resolveCourierLivePosition(orderAttrs, courierAttrs) {
    const delivery = orderAttrs?.delivery;
    if (delivery && hasValidGpsPair(delivery.courierLatitude, delivery.courierLongitude)) {
        const lat = toNum(delivery.courierLatitude);
        const lng = toNum(delivery.courierLongitude);
        if (!orderAttrs || gpsPlausibleForOrder(lat, lng, orderAttrs)) {
            return { lat, lng };
        }
    }
    if (courierAttrs && hasValidGpsPair(courierAttrs.lat, courierAttrs.lng)) {
        const lat = toNum(courierAttrs.lat);
        const lng = toNum(courierAttrs.lng);
        if (!orderAttrs || gpsPlausibleForOrder(lat, lng, orderAttrs)) {
            return { lat, lng };
        }
    }
...
function buildPublicCourier(entry, assignedCount = 0) {
    const attrs = entry ?? {};
    const state = mapCourierStatus(attrs.courier_status, attrs.isOnline);
    const profileGps = resolveCourierLivePosition(null, attrs);
 ...
 */
function buildCourierDeliverySummary(rawOrders) {
    if (!Array.isArray(rawOrders))
        return [];
    return rawOrders
        .filter((o) => mapOrderStatus(o?.order_status) === 'DELIVERED')
        .sort((a, b) => {
 ...
 */
function buildCustomerOrderSummary(rawOrders) {
    if (!Array.isArray(rawOrders))
        return [];
    return [...rawOrders]
...
function parsePublicId(rawId, prefix) {
    const clean = String(rawId || '').trim();
    if (clean.startsWith(`${prefix}-`))
        return Number(clean.slice(prefix.length + 1));
    return Number(clean);
}
...
function scoreCourierForOrder(order, courier) {
    let score = 0;
    const reasons = [];
    ...
    if (courier.zones?.includes?.(order.zone)) {
        ...
        return { score: 0, reasons: ['fora_da_zona'], distanceKm: null };
    }
...
function computeSlaMetrics(rawOrders) {
    ...
    for (const row of rows) {
        ...
        if (st === 'PENDING')
            pendingMs.push(ageSinceCreate);
        if (st === 'APPROVED')
            approvedMs.push(ageSinceCreate);
        if (st === 'ASSIGNED' || st === 'IN_TRANSIT' || st === 'DELIVERED')
            assignedMs.push(ageSinceCreate);
...
function auditSnapshotCourier(c) {
    if (!c)
        return null;
    return c;
}
function mapContinentStore(entry) {
    if (!entry)
        return null;
    return {
...
function mapCatalogProduct(entry) {
    if (!entry)
        return null;
    return {
...
function buildOperationalAdminAlerts(orders, couriers, opts) {
    const now = Date.now();
    ...
    for (const o of orders) {
        ...
        if (String(o.deliveryStatus || '').startsWith('E-14')) {
            alerts.push({
                id: `bo-impossible-${o.id}`,
                at: new Date().toISOString(),
                level: 'urgent',
                message: `Pedido ${o.id}: estafeta reportou impossível antes da recolha. Reatribuir estafeta ou cancelar no painel.`,
            });
        }
    }
...
exports.default = ({ strapi }) => ({
    ...
    async loginWithEmail(payload) {
        ...
        if (!email || !password)
            return { ok: false, error: 'Email e password são obrigatórios.' };
    ...
    async loginWithGoogle(payload) {
        const email = String(payload.email || '').trim().toLowerCase();
        if (!email)
            return { ok: false, error: 'Payload Google inválido (email em falta).' };
        let user = (await strapi.db.query('plugin::users-permissions.user').findMany({
            where: { email },
    ...
    async getOrdersMapped() {
        ...
        return entries.map((e) => buildPublicOrder(e));
    },
    async getCouriersMapped() {
        ...
        for (const o of orders) {
            if (!o.courierId)
                continue;
            if (!['ASSIGNED', 'IN_TRANSIT'].includes(o.status))
                continue;
        ...
        return entries.map((e) => {
            ...
            if (cur) {
                ...
                const liveOrder = orders.find((o) => o.id === cur.id);
                if (liveOrder?.courierLiveLat != null && liveOrder?.courierLiveLng != null) {
                    built.lat = liveOrder.courierLiveLat;
                    built.lng = liveOrder.courierLiveLng;
                }
            }
            return built;
        });
    ...
    async getCatalogProducts() {
        ...
        return rows.map((r) => mapCatalogProduct(r)).filter((r) => !!r);
    },
    async upsertCatalogProduct(payload) {
        const sku = String(payload.sku || '').trim().toUpperCase();
        const name = String(payload.name || '').trim();
        if (!sku || !name)
            return { ok: false, error: 'SKU e nome são obrigatórios.' };
    ...
    async deleteCatalogProduct(publicIdOrSku) {
        ...
        if (!target && Number.isFinite(numeric))
            target = await strapi.db.query('api::catalog-product.catalog-product').findOne({ where: { id: numeric } });
        if (!target) {
    ...
    async ensureContinentStoresSeeded() {
        ...
        for (const row of continent_stores_seed_js_1.CONTINENTE_STORES_SEED) {
    ...
    async getContinentStores() {
        ...
        return rows.map((r) => mapContinentStore(r)).filter(Boolean);
    },
    async createContinentStore(ctx, payload) {
        const code = String(payload.code || '').trim().toUpperCase();
        const name = String(payload.name || '').trim();
        if (!code || !name)
            return { ok: false, error: 'Código e nome da loja são obrigatórios.' };
        const existing = await strapi.db.query('api::continent-store.continent-store').findOne({ where: { code } });
        if (existing)
            return { ok: false, error: 'Já existe uma loja com esse código.' };
    ...
    async updateContinentStore(ctx, publicId, payload) {
        ...
        if (!target && Number.isFinite(numeric))
            target = await strapi.db.query('api::continent-store.continent-store').findOne({ where: { id: numeric } });
        if (!target)
        ...
        if (!nextCode)
            return { ok: false, error: 'Código da loja obrigatório.' };
        if (nextCode !== target.code) {
            ...
            if (codeTaken && codeTaken.id !== target.id)
                return { ok: false, error: 'Código já em uso por outra loja.' };
        }
    ...
    async deleteContinentStore(ctx, publicId) {
        const token = parsePublicToken(publicId, 'CT');
        const numeric = Number(token);
        let target = await strapi.db.query('api::continent-store.continent-store').findOne({ where: { documentId: token } });
        if (!target && Number.isFinite(numeric))
        ...
        if (linkedInventory)
            return { ok: false, error: 'Loja com stock associado. Remove os itens antes de eliminar.' };
        const before = mapContinentStore(target);
        await strapi.db.query('api::continent-store.continent-store').delete({ where: { id: target.id } });
        return { ok: true, data: { id: publicId } };
    },
    async getStoreInventory(publicStoreId, filters) {
        ...
        if (!store && Number.isFinite(numeric))
            store = await strapi.db.query('api::continent-store.continent-store').findOne({ where: { id: numeric } });
        if (!store)
            return { ok: false, error: 'Loja não encontrada.' };
        ...
        let mapped = rows.map((r) => mapInventoryItem(r));
        const q = String(filters?.q || '').trim().toLowerCase();
        if (q) {
            mapped = mapped.filter((r) => `${r.sku} ${r.name} ${r.brand} ${r.category}`.toLowerCase().includes(q));
        }
        return { ok: true, data: mapped };
    },
    async upsertInventoryItem(ctx, publicStoreId, payload) {
        const token = parsePublicToken(publicStoreId, 'CT');
        const numeric = Number(token);
        let store = await strapi.db.query('api::continent-store.continent-store').findOne({ where: { documentId: token } });
        if (!store && Number.isFinite(numeric))
        ...
        const name = String(payload.name || '').trim();
        if (!sku || !name)
            return { ok: false, error: 'SKU e nome do item são obrigatórios.' };
    ...
    async deleteInventoryItem(ctx, publicStoreId, publicItemId) {
        ...
        if (!target)
            return { ok: false, error: 'Item de stock não encontrado.' };
    ...
    async listOrders(filters) {
        ...
        return rows.filter((o) => {
            if (byStatus && o.status !== byStatus)
                return false;
            if (filters.priority && String(o.priority) !== String(filters.priority))
                return false;
            if (filters.type && o.type !== filters.type)
                return false;
            if (filters.zone && o.zone !== filters.zone)
                return false;
            if (filters.q) {
    ...
    async getOrder(ctx, publicId) {
        const rows = await this.getOrdersMapped();
        const order = rows.find((o) => o.id === publicId || String(o._documentId) === publicId) ?? null;
        if (!order)
            return null;
    ...
    async patchOrder(publicId, patch, eventInput) {
        ...
        const mergedBoMeta = { ...existingBoMeta, ...patchBoMeta };
        if (eventInput) {
        ...
        (0, bo_event_bus_js_1.emitBoChange)({ entity: 'order', id: target.documentId, action: 'update', meta: { event: eventInput?.action } });
        return updated;
    },
    async notify(type, message, userId) {
        try {
    ...
    async trySendEmail(to, subject, text) {
        ...
        emailService.send({ to, subject, text }).catch((err) => {
            console.error('[Email Error]', err.message);
        });
    },
    async approveOrder(ctx, publicId, payload) {
        ...
        const store = stores.find((s) => s.id === payload.storeId || s.code === payload.storeId);
    ...
    async rejectOrder(ctx, publicId, payload) {
        const reason = String(payload.justification || payload.reason || '').trim();
        if (!reason)
            return { ok: false, error: 'Justificação obrigatória.' };
        const before = auditSnapshotOrder(await this.getOrder(ctx, publicId));
        const order = before;
        if (!order)
            return { ok: false, error: 'Pedido não encontrado.' };
    ...
    async assignCourier(ctx, publicId, payload) {
        ...
        if (!updated)
            return { ok: false, error: 'Falha ao atualizar estado da encomenda.' };
    ...
    async setPriority(ctx, publicId, payload) {
        ...
        if (!updated)
            return { ok: false, error: 'Falha ao atualizar prioridade.' };
        if (priority === 5) {
    ...
    async startTransit(ctx, publicId) {
        const before = auditSnapshotOrder(await this.getOrder(ctx, publicId));
        const order = before;
        if (!order)
            return { ok: false, error: 'Pedido não encontrado.' };
        if (order.status !== 'ASSIGNED')
            return { ok: false, error: 'Apenas pedidos atribuídos podem iniciar trânsito.' };
        ...
        if (!updated)
            return { ok: false, error: 'Falha ao iniciar trânsito.' };
        const after = buildPublicOrder(updated);
        return { ok: true, data: after };
    },
    async completeOrder(ctx, publicId) {
        const before = auditSnapshotOrder(await this.getOrder(ctx, publicId));
        const order = before;
        if (!order)
            return { ok: false, error: 'Pedido não encontrado.' };
        if (order.status !== 'IN_TRANSIT')
            return { ok: false, error: 'Apenas pedidos em trânsito podem ser concluídos.' };
        ...
        if (!updated)
            return { ok: false, error: 'Falha ao concluir pedido.' };
        const after = buildPublicOrder(updated);
        return { ok: true, data: after };
    },
    async setOrderDeliveryStatus(ctx, publicId, payload) {
        ...
        const order = await this.getOrder(ctx, publicId);
        if (!order?._documentId)
            return { ok: false, error: 'Pedido não encontrado.' };
        if ((0, order_state_machine_js_1.isOrderTerminal)(order.status)) {
        ...
        if (!target?.documentId)
            return { ok: false, error: 'Delivery não encontrada para este pedido.' };
        ...
        if (!(0, order_state_machine_js_1.canTransitionDelivery)(currentDeliveryStatus, nextStatus)) {
        ...
        const nextOrderStatus = order_state_machine_js_1.DELIVERY_TO_ORDER_STATUS[nextStatus];
        if (nextOrderStatus) {
    ...
    async cancelOrderByAdmin(ctx, publicId, payload) {
        ...
        if (!updated)
            return { ok: false, error: 'Falha ao cancelar pedido.' };
    ...
    async patchOrderAdmin(ctx, publicId, payload) {
        const before = auditSnapshotOrder(await this.getOrder(ctx, publicId));
        const order = before;
        if (!order)
            return { ok: false, error: 'Pedido não encontrado.' };
        ...
        data.items = newItems;
        const updated = await this.patchOrder(publicId, { data }, {
            action: 'admin_patch',
            ctx,
    ...
    async listAppNotifications(_ctx, query) {
    ...
    async markAppNotificationRead(_ctx, documentId) {
        ...
        try {
            ...
            (0, bo_event_bus_js_1.emitBoChange)({ entity: 'notification', id: clean, action: 'read' });
            return { ok: true };
        }
        catch {
    ...
    async getCourier(publicId) {
        const rows = await this.getCouriersMapped();
        return rows.find((c) => c.id === publicId || String(c._documentId) === publicId) ?? null;
    },
    async createCourier(ctx, payload) {
        const name = String(payload.name || '').trim();
        if (!name)
            return { ok: false, error: 'Nome obrigatório.' };
        const emailRes = (0, back_office_validation_js_1.validateEmail)(payload.email);
        if (!emailRes.ok)
            return { ok: false, error: emailRes.error };
        const nifRes = (0, back_office_validation_js_1.validatePtNif)(payload.nif);
        if (!nifRes.ok)
            return { ok: false, error: nifRes.error };
        const ibanRes = (0, back_office_validation_js_1.validatePtIban)(payload.iban);
        if (!ibanRes.ok)
            return { ok: false, error: ibanRes.error };
        const phoneRes = (0, back_office_validation_js_1.validatePtPhone)(payload.phone);
        if (!phoneRes.ok)
            return { ok: false, error: phoneRes.error };
        const plateRaw = payload.plate ?? payload.vehicle?.plate;
        const plateRes = (0, back_office_validation_js_1.validatePtPlate)(plateRaw);
        if (!plateRes.ok)
            return { ok: false, error: plateRes.error };
        ...
        (0, bo_event_bus_js_1.emitBoChange)({ entity: 'courier', id: created.documentId, action: 'create' });
        return { ok: true, data: pub };
    },
    async updateCourier(ctx, publicId, payload) {
        ...
        const emailRes = (0, back_office_validation_js_1.validateEmail)(nextEmail);
        if (!emailRes.ok)
            return { ok: false, error: emailRes.error };
        const nextNif = payload.nif != null ? String(payload.nif).trim() : target.nif;
        const nifRes = (0, back_office_validation_js_1.validatePtNif)(nextNif);
        if (!nifRes.ok)
            return { ok: false, error: nifRes.error };
        const nextIban = payload.iban != null ? String(payload.iban).trim() : target.iban;
        const ibanRes = (0, back_office_validation_js_1.validatePtIban)(nextIban);
        if (!ibanRes.ok)
            return { ok: false, error: ibanRes.error };
        const nextPhone = payload.phone != null ? String(payload.phone).trim() : target.phone;
        const phoneRes = (0, back_office_validation_js_1.validatePtPhone)(nextPhone);
        if (!phoneRes.ok)
            return { ok: false, error: phoneRes.error };
        ...
        const updated = await strapi.documents('api::courier-estafeta.courier-estafeta').update({
            ...
            data: {
                ...
                address: payload.address ?? target.address,
                iban: ibanRes.value,
                maxSimultaneousDeliveries: payload.maxConcurrent != null ? Math.max(1, toNum(payload.maxConcurrent, 1)) : target.maxSimultaneousDeliveries,
        ...
        (0, bo_event_bus_js_1.emitBoChange)({ entity: 'courier', id: target.documentId, action: 'update' });
        return { ok: true, data: after };
    },
    async courierAction(ctx, publicId, payload) {
        ...
        if (action === 'verify') {
            ...
            patch.courier_status = shouldOnline ? COURIER_STATE.E06 : 'E-07 Em Pausa';
        }
        else if (action === 'change_status') {
            const s = String(payload.status || '');
            if (s === 'E-01') {
                patch.courier_status = COURIER_STATE.E01;
                patch.isOnline = false;
            }
            else if (s === 'E-02') {
                patch.courier_status = COURIER_STATE.E02;
                ...
                patch.courier_status = COURIER_STATE.E06;
                patch.isOnline = true;
            }
            else if (s === 'E-07') {
                patch.courier_status = 'E-07 Em Pausa';
                patch.isOnline = false;
            }
        }
        ...
        const after = buildPublicCourier(updated);
        (0, bo_event_bus_js_1.emitBoChange)({ entity: 'courier', id: target.documentId, action: 'transition', meta: { action } });
        return { ok: true, data: after };
    },
    async getOperationsMap() {
        const orders = await this.getOrdersMapped();
        const couriers = await this.getCouriersMapped();
        const activeOrders = orders.filter((o) => ['APPROVED', 'ASSIGNED', 'IN_TRANSIT'].includes(o.status));
        const stores = await this.getContinentStores();
        return {
            stores: stores.filter((s) => s.isActive),
            couriers: couriers.filter((c) => c.online || c.state === 'E-06' || c.state === 'E-07'),
            activeOrders,
        };
    },
    async getCustomers() {
        ...
        return users.map((u) => {
            ...
            const delivered = orders.filter((o) => mapOrderStatus(o.order_status) === 'DELIVERED');
            const totalSpent = delivered.reduce((acc, o) => acc + toNum(o.total_price, 0), 0);
            const lastOrderAt = orders.length ? orders.map((o) => o.createdAt).sort().slice(-1)[0] : null;
            const ratings = (u.reviews || []).map((r) => toNum(r.rating, 0)).filter((n) => n > 0);
    ...
    async createCustomer(ctx, payload) {
        ...
        let nifVal = '';
        if (nifRaw) {
            const nifRes = (0, back_office_validation_js_1.validatePtNif)(nifRaw);
            if (!nifRes.ok)
                return { ok: false, error: nifRes.error };
            nifVal = nifRes.value;
        }
        ...
        const mapped = rows.find((r) => r.id === idToPublic('CU', created.id));
        (0, bo_event_bus_js_1.emitBoChange)({ entity: 'customer', id: created.id, action: 'create' });
        return { ok: true, data: mapped || null };
    },
    async updateCustomer(ctx, publicId, payload) {
        ...
        const rows = await this.getCustomers();
        const mapped = rows.find((r) => r.id === idToPublic('CU', userId));
        (0, bo_event_bus_js_1.emitBoChange)({ entity: 'customer', id: userId, action: 'update' });
        return { ok: true, data: mapped || null };
    },
    async deleteCustomer(ctx, publicId) {
        const token = parsePublicToken(publicId, 'CU');
        const userId = Number(token);
        if (!Number.isFinite(userId))
            return { ok: false, error: 'Cliente inválido.' };
        ...
        (0, bo_event_bus_js_1.emitBoChange)({ entity: 'customer', id: userId, action: 'delete' });
    ...
    async getPublicMetrics() {
        ...
        const ratedOrders = rawOrders.filter((r) => r.rating && r.rating > 0);
        const avgRating = ratedOrders.length > 0
            ? (ratedOrders.reduce((acc, r) => acc + r.rating, 0) / ratedOrders.length).toFixed(1)
            : '4.9';
    ...
    async chatWithBot(body) {
        const userMessage = body?.message || '';
        const history = body?.history || [];
        if (!userMessage)
            return { error: 'A mensagem está vazia.' };
        const apiKey = process.env.GEMINI_API_KEY;
        if (!apiKey)
            return { error: 'O assistente encontra-se temporariamente indisponível.' };
        let conversation = history.map((m) => `${m.role === 'user' ? 'Cliente' : 'Assistente'}: ${m.text}`).join('\n');
        if (!conversation)
            conversation = `Cliente: ${userMessage}`;
        const prompt = `
...
Responde apenas com a tua próxima fala.
    ...
    async getDashboard() {
        ...
        return {
            kpiSummary: {
                ordersToday: orders.filter((o) => String(o.createdAt || '').startsWith(today)).length,
                active,
                online: couriers.filter((c) => c.online).length,
                revenuePipeline: orders
                    .filter((o) => ['APPROVED', 'ASSIGNED', 'IN_TRANSIT'].includes(o.status))
                    .reduce((a, o) => a + toNum(o.costEuro, 0), 0),
            ...
            deliveriesByZone: Object.entries(deliveriesByZone)
                .map(([zone, count]) => ({ zone, count }))
                .sort((a, b) => b.count - a.count),
            recentActivity: orders.slice(0, 10).map((o) => ({
                id: `act-${o.id}`,
                at: o.createdAt,
    ...
    async getReports() {
        ...
        const cancelled = orders.filter((o) => o.status === 'REJECTED' || o.status === 'CANCELLED_ADMIN').length;
    ...
    async getBootstrapData() {
        ...
        const dashboard = {
            ...
            recentReviews: [],
            kpiSummary: {
                ordersToday: orders.filter((o) => String(o.createdAt || '').startsWith(new Date().toISOString().slice(0, 10))).length,
